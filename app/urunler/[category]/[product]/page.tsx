...
export async function generateMetadata({ params }: Props): Promise<Metadata> { const { category: categorySlug, product: productSlug } = await params; const product = getProduct(categorySlug, productSlug); return { title: product ? product.name + " | M&Y Kozmetik" : "ÃœrÃ¼nler | M&Y Kozmetik", description: product?.shortDescription }; }
